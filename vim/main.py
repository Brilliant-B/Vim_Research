...
def main(args):
    ...
    train_or_test = 'test' if args.eval else 'train'
    ...
    if args.local_rank == 0 and (args.eval or args.gpu == 0):
    ...
    if args.finetune:
        ...
        # remove immatchable params
        for k in ['head.weight', 'head.bias', 'head_dist.weight', 'head_dist.bias', 'rope.freqs_cos', 'rope.freqs_sin']:
            if k in checkpoint_model and checkpoint_model[k].shape != state_dict[k].shape:
                print(f"Removing key {k} from pretrained checkpoint")
                del checkpoint_model[k]

        # interpolate position embedding
        checkpoint_model = utils.interpolate_pos_embed_vim(model, checkpoint_model)
    ...
    with open(os.path.join(args.output_dir, f'{train_or_test}_log.log'), 'a+') as f:
        if utils.is_main_process():
            if not args.eval:
                print(f'train batch size = {utils.get_world_size() * args.batch_size}', file=f)
            print(f'val batch size = {utils.get_world_size() * int(1.5 * args.batch_size)}', file=f)
            print(f'total params = {n_parameters}', file=f)
        if not args.eval:
            print(f'train batch size = {utils.get_world_size() * args.batch_size}')
        print(f'val batch size = {utils.get_world_size() * int(1.5 * args.batch_size)}')
        print(f'total params = {n_parameters}')
    ...
    if utils.is_main_process():
        with open(os.path.join(args.output_dir, 'train_log.log'), 'a+') as f:
            print(f"Start training for {args.epochs} epochs", file=f)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and utils.is_main_process():
            with open(os.path.join(output_dir, "train_log.log"), "a+") as f:
    ...
    with open(os.path.join(args.output_dir, 'train_log.log'), 'a+') as f:
        if utils.is_main_process():
            print(f'Training time {total_time_str}', file=f)
