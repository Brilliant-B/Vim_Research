...
class Block(nn.Module):
    ...
    def forward(
        self, hidden_states: Tensor, residual: Optional[Tensor] = None, inference_params=None, post_cls = False
    ):
        ...
        hidden_states = self.mixer(hidden_states, inference_params=inference_params, post_cls=post_cls)
...
def vim_pcls_tiny_bimambav2_final_pool_mean_abs_pos_embed_rope_also_residual_with_cls_token(pretrained=False, **kwargs):
    model = VisionMamba_HI(
        post_cls=True, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", if_cls_token=True, **kwargs)
